...
import { getDeveloperBySlug, getDeveloperProjects } from '@/lib/developerService';
import { getDictionary } from '@/lib/i18n';
import { tx } from '@/lib/i18n-utils';
import Image from 'next/image';
import Link from 'next/link';
import ProjectCard from '@/components/ProjectCard';
import { Building2, MapPin, Calendar, Award, TrendingUp } from 'lucide-react';
...
export default async function DeveloperPage({ params }: DeveloperPageProps) {
  ...
  // Extract unique areas/communities - handle both string and localized object
  const uniqueAreas = Array.from(
    new Set(
      projects
        .map(p => {
          const area = p.area || p.location;
          return area ? tx(area, locale as 'ar' | 'en') : null;
        })
        .filter(Boolean)
    )
  ) as string[];
  ...
  const stats = {
    ...
    activeProjects: projects.filter(p => p.projectStatus === 'under-construction' || tx(p.projectStatus, locale as 'ar' | 'en')?.toLowerCase().includes('construction')).length,
    completedProjects: projects.filter(p => p.projectStatus === 'ready' || tx(p.projectStatus, locale as 'ar' | 'en')?.toLowerCase().includes('ready')).length,
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-black">
      ...
      <div className="relative h-[400px] overflow-hidden">
        ...
        <div 
          ...
          style={{
            backgroundImage: `url(${developer.logo || '/media/default-developer.jpg'})`,
              ...
              {uniqueAreas.map((area) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-400">
                    {projects.filter(p => {
                      const projectArea = p.area || p.location;
                      return projectArea && tx(projectArea, locale as 'ar' | 'en') === area;
                    }).length} {isRTL ? 'مشاريع' : 'projects'}
                  </div>
                </div>
              ))}
            </div>
