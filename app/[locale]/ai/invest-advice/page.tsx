...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: dict.ai.investmentTips,
    description: dict.ai.investmentTipsDesc,
  };
}

export default async function InvestAdvicePage({ params }: Props) {
  const { locale } = await params;
  const dict = await getDictionary(locale);
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-zinc-900/50 border border-zinc-700/50 rounded-xl p-8 text-gray-200">
          {/* Placeholder: render investment tips UI here */}
          <p className="mb-2">{locale === 'ar' ? 'نصائح الاستثمار قادمة قريبًا.' : 'Investment advice UI coming soon.'}</p>
        </div>
      </div>
    </div>
  );
